...
describe 'delivery-truck::unit' do
  ...
  context 'when a single cookbook has been modified' do
    before do
      ...
      allow(DeliveryTruck::Helpers::Unit).to receive(:spec_tests?).with('/tmp/repo/cookbooks/julia').and_return(true)
    end
  ...
  context 'when multiple cookbooks have been modified' do
    before do
      ...
      allow(DeliveryTruck::Helpers::Unit).to receive(:spec_tests?).with('/tmp/repo/cookbooks/julia').and_return(true)
      allow(DeliveryTruck::Helpers::Unit).to receive(:spec_tests?).with('/tmp/repo/cookbooks/gordon').and_return(true)
    end
