...
describe 'delivery-truck::functional', ignore: true do
  ...
  context 'when a single cookbook has been modified' do
    before do
      ...
      allow(DeliveryTruck::Helpers::Functional).to receive(:kitchen_tests?).with('/tmp/cookbooks/julia').and_return(true)
    end
  ...
  context 'when multiple cookbooks have been modified' do
    before do
      ...
      allow(DeliveryTruck::Helpers::Functional).to receive(:kitchen_tests?).with('/tmp/cookbooks/julia').and_return(true)
      allow(DeliveryTruck::Helpers::Functional).to receive(:kitchen_tests?).with('/tmp/cookbooks/gordon').and_return(true)
    end
    ...
    context 'but a cookbook has no tests' do
      before do
        allow(DeliveryTruck::Helpers::Functional).to receive(:kitchen_tests?).with('/tmp/cookbooks/gordon').and_return(false)
      end
